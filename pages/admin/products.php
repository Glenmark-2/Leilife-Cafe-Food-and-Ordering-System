   ...
   function toggleEdit(btn) {
    ...
    if (!isEditing) {
        ...
        // Save changes (your existing fetch logic)
        const updatedData = {
        ...
        .then(data => {
            if (data.success) {
                ...
                [...categorySelect.options].forEach(opt => {
                    if (opt.value === updated.main_category_name) opt.selected = true;
                });
                statusBtn.textContent = updated.status;

                // Disable again & remove highlight
                nameInput.disabled = true;
                priceInput.disabled = true;
                categorySelect.disabled = true;
                statusBtn.disabled = true;
    ...
    document.getElementById("add").addEventListener("click", () => {
        ...
        .then(data => {
            if (data.success) {
                ...
                setTimeout(() => {
            location.reload();
        }, 1000); // delay para makita yung success modal
