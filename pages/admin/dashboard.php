...
function attachItemDelegatedListener(container, orderId) {
  ...
  const listener = async (e) => {
    ...
    if (newStatus === 'cancelled') {
      showConfirmModal("Are you sure you want to cancel this item?", async () => {
        await updateItemStatus(select, tr, itemId, prev, newStatus, feedback);
      }, () => {
        // user pressed cancel → revert immediately
        select.value = prev;
      });
    } else {
      // Normal flow for non-cancelled
      await updateItemStatus(select, tr, itemId, prev, newStatus, feedback);
    }
  };
...
async function updateItemStatus(select, tr, itemId, prev, newStatus, feedback) {
  select.disabled = true;
  ...
  try {
    ...
    if (result.success) {
      ...
      // 🧱 2️⃣ If cancelled → lock permanently
      if (newStatus === 'cancelled') {
        select.disabled = true;
      } else {
