...
<!-- Cancel Reason Modal -->
<div id="cancelReasonModal" style="display:none; position:fixed; inset:0;
    background:#0007; justify-content:center; align-items:center; z-index:99999;">
  <div style="background:#fff; padding:20px; width:320px; border-radius:10px;">
    <h3 style="margin-bottom:10px;">Cancel Order</h3>
    <p style="margin-bottom:8px;">Optional Reason:</p>
    <textarea id="cancelReasonInput"
      style="width:100%; height:70px; resize:none; border:1px solid #ddd; border-radius:6px;
      padding:5px;"></textarea>

    <div style="display:flex; gap:10px; margin-top:15px; justify-content:flex-end;">
      <button id="cancelReasonSubmit"
        style="padding:6px 12px; background:var(--accent); border:none; color:#fff; border-radius:6px;">OK</button>
      <button id="cancelReasonClose"
        style="padding:6px 12px; background:#eee; border:1px solid #ccc; border-radius:6px;">Close</button>
    </div>
  </div>
</div>
...
function disableCancelledItemDropdowns(container = document) {
  container.querySelectorAll('.item-status').forEach(select => {
...
orders.forEach(order => {
  ...
  <p class="col-payment">
    ${order.payment_status
      ? escapeHtml(order.payment_status.toLowerCase().replace(/\b\w/g, c => c.toUpperCase()))
      : 'Undefined'}
  </p>

  <p class="col-method">${escapeHtml(order.delivery_method)? escapeHtml(order.delivery_method.toLowerCase().replace(/\b\w/g, c => c.toUpperCase()))
    : 'Undefined'}</p>
...
disableCancelledItemDropdowns(expandRow);
...
function attachStatusListeners() {
  ...
  document.querySelectorAll('.status-option').forEach(opt => {
    opt.addEventListener('click', async (e) => {
      ...
      const confirmAndDo = async (reasonText = "") => {
        ...
        try {
          ...
          if (data.success) {
            ...
            if (expandRow) {
  expandRow.querySelectorAll('tr[data-item-id]').forEach(tr => {
    const sel = tr.querySelector('.item-status');
    if (sel && sel.value !== 'cancelled') { // only update non-cancelled
      const mapped = mapOrderToItemStatus(newStatus);
      sel.value = mapped;
      sel.dataset.prev = mapped;
      tr.className = `status-${escapeCss(mapped)}`;
    }
  });
}
...
if (newStatus === 'cancelled') {
  showConfirmModal("Are you sure you want to cancel this order?", () => {
    // ask optional reason
    showCancelReasonModal(orderId, newStatus, (reasonText) => {
      confirmAndDo(reasonText);
    });
  });
} else {
  confirmAndDo();
...
function attachItemDelegatedListener(container, orderId) {
  ...
  const listener = async (e) => {
    ...
    // 🧠 1️⃣ Ask confirmation if cancelling
if (newStatus === 'cancelled') {
  showConfirmModal("Are you sure you want to cancel this item?", () => {
    // after confirm → show same reason modal used by order-level
    showCancelReasonModal(itemId, newStatus, async (reasonText) => {
      await updateItemStatus(select, tr, itemId, prev, newStatus, feedback, reasonText);
    });
  }, () => {
    // if cancelled
    select.value = prev;
  ...
  await updateItemStatus(select, tr, itemId, prev, newStatus, feedback);
}
  };
...
/* Helper for status update logic (shared by both paths) */
async function updateItemStatus(select, tr, itemId, prev, newStatus, feedback, reasonText = "") {
  select.disabled = true;
  ...
  try {
    const res = await fetch('/Leilife/backend/admin/update_order_item_status.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ order_item_id: itemId, status: newStatus, reason: reasonText })
    });
    ...
    if (result.success) {
      ...
      // Lock cancelled item permanently
      if (newStatus === 'cancelled') select.disabled = true;
      else select.disabled = false;

      // 🔍 NEW FEATURE: Check if all items in the order are cancelled
      const expandRow = tr.closest('.expandable-row');
      const orderId = expandRow?.dataset.orderId;

      if (orderId && expandRow) {
        const itemRows = expandRow.querySelectorAll('tr[data-item-id]');
        const allCancelled = Array.from(itemRows).every(row => {
          const s = row.querySelector('.item-status');
          return s && s.value === 'cancelled';
        });

        if (allCancelled) {
          console.log(`🧩 All items in order ${orderId} are cancelled. Auto-updating order...`);

          try {
            const orderRes = await fetch('/Leilife/backend/admin/update_order_status.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            if (orderData.success) {
              console.log(`✅ Order ${orderId} auto-updated to cancelled`);

              // Update the main row visually
              const mainRow = document.querySelector(`.table-row[data-order-id="${orderId}"]`);
              if (mainRow) {
                // Update the badge text + class
                const badge = mainRow.querySelector('.badge');
                if (badge) {
                  badge.textContent = 'Cancelled';
                  badge.className = 'badge s-cancelled';
                }

                // Update the status button dataset
                const statusBtn = mainRow.querySelector('.status-btn');
                if (statusBtn) {
                  statusBtn.dataset.status = 'cancelled';
                }

                // Disable any open menu (if needed)
                const statusMenu = mainRow.querySelector('.status-menu');
                if (statusMenu) {
                  statusMenu.setAttribute('aria-hidden', 'true');
                }
              }
            } else {
              console.warn(`⚠️ Backend failed to auto-cancel order ${orderId}`);
            }

          } catch (err) {
            console.error(`⚠️ Network error while updating order ${orderId} to cancelled:`, err);
          }
        }
      }
...
function showCancelReasonModal(orderId, newStatus, callback) {
  __cancelTemp.orderId   = orderId;
  __cancelTemp.newStatus = newStatus;
  __cancelTemp.callback  = callback;

  const modal = document.getElementById("cancelReasonModal");
  const input = document.getElementById("cancelReasonInput");

  if (!modal || !input) return callback("");

  input.value = "";
  modal.style.display = "flex";

  // short-lived → auto close after 30s
  if (modal.__timeout) clearTimeout(modal.__timeout);
  modal.__timeout = setTimeout(() => {
    modal.style.display = "none";
    callback(input.value.trim());   // even if blank
  }, 30000);
}

document.getElementById("cancelReasonSubmit")?.addEventListener("click", () => {
  const modal = document.getElementById("cancelReasonModal");
  const input = document.getElementById("cancelReasonInput");
  modal.style.display = "none";
  if (__cancelTemp.callback) __cancelTemp.callback(input.value.trim());
});

document.getElementById("cancelReasonClose")?.addEventListener("click", () => {
  const modal = document.getElementById("cancelReasonModal");
  modal.style.display = "none";
  if (__cancelTemp.callback) __cancelTemp.callback("");
});
